...
/><ApiField
  ...
  type="string array"
  required="false"
  description="HeadersToForward specifies which headers from the original client request should be forwarded to this backend.<br />Headers are case-insensitive per HTTP spec. If a header doesn't exist in the request, it is silently ignored.<br />Conflict detection: If APIKey is configured and headersToForward includes the APIKey injection header<br />(default `Authorization` or custom via APIKey.Header), the MCPRoute will be rejected with NotAccepted status."
/>
